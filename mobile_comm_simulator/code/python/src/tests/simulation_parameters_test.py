...
# @pytest.mark.skip
def test_simulation_parameters_are_created(
    ...
    assert (
        simulation_parameters.get_simulation_parameters().array_elements_spacing
        == "uniform"
    )
...
def test_num_snapshots(num_snapshots, expected):
    ...
    if expected:
        ...
        simul_params = SimulationParameters(
            SimulationConfig(num_snapshots=num_snapshots)
        )
...
def test_invalid_num_snapshots():
    ...
    with pytest.raises(ValueError, match="Number of snapshots must be greater than 1"):
        SimulationParameters(
            SimulationConfig(num_snapshots=invalid_config["num_snapshots"])
        )
...
def test_invalid_num_sensors():
    ...
    with pytest.raises(ValueError, match="Number of sensors must be greater than 1"):
        SimulationParameters(
            SimulationConfig(num_sensors=invalid_config["num_sensors"])
        )
...
def test_invalid_center_frequency():
    ...
    with pytest.raises(ValueError, match="Center frequency must be non-negative"):
        SimulationParameters(
            SimulationConfig(center_frequency=invalid_config["center_frequency"])
        )
...
def test_invalid_source_angles():
    ...
    with pytest.warns(
        ...
        SimulationParameters(
            SimulationConfig(source_angles=invalid_config["source_angles"])
        )
...
def test_invalid_source_powers():
    ...
    with pytest.raises(
        ValueError, match="Number of source powers must match number of angles"
    ):
        SimulationParameters(
            SimulationConfig(
                source_powers=invalid_config["source_powers"], source_angles=[0]
            )
        )
...
def test_invalid_spacing():
    ...
    with pytest.raises(
        ValueError,
        match=f"Invalid enum value: '{invalid_config['array_elements_spacing']}' is not a valid SpacingType",
    ):
        SimulationParameters(
            SimulationConfig(
                array_elements_spacing=invalid_config["array_elements_spacing"]
            )
        )
