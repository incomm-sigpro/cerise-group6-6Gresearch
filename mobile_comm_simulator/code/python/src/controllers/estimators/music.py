...
class MusicController:
    def __init__(self, num_antennas, num_snapshots, num_signals, wavelength, spacing, angles):
        ...
        self.angles = angles
        self.model = SignalModel(num_antennas, num_snapshots, num_signals, wavelength, spacing, angles)
    ...
    def run(self):
        ...
        for i, angle in enumerate(angles_scan):
            a = np.exp(-1j * 2 * np.pi * self.spacing * np.arange(self.num_antennas) * np.sin(np.deg2rad(angle)) / self.wavelength)
