...
class TestMath(unittest.TestCase):
    ...
    def assert_unique_rows(self, x, y, atol, rtol):
        ...
        for i in range(n):
            for k in range(i + 1, n):
                self.assertTrue(
                    np.any(np.abs(y[i, :] - y[k, :]) > tol),
                    "Difference too small when comparing {0} and {1}".format(
                        y[i, :], y[k, :]
                    ),
                )
    ...
    def test_unique_rows(self):
        ...
        np.random.seed(1123)
        centers = np.array([[0, -1, 1], [0, 1, -1], [1, 1, 1]])
        data3 = np.vstack(
            [
...
class TestConversion(unittest.TestCase):
    ...
    def test_cartesian_to_spherical(self):
        ...
        s1_expected = np.array([[1.0, 0.0, 0.0], [2.0, np.pi, 0.0], [3.0, 0.0, 0.0]])
        npt.assert_allclose(doa_conv.cart2spherical(coords1), s1_expected)
        # 2D
        coords2 = np.array([[1.0, 1.0], [0.0, -1.0], [-2.0, -2.0]])
        ...
        # 3D
        coords3 = np.array(
            [[0.0, 0.0, 1.0], [-1.0, 1.0, 1.0], [3 / 4, np.sqrt(3) / 4, 1 / 2]]
        )
