...
from doatools.model.sources import (
...
class TestFarField1D(unittest.TestCase):
    ...
    def test_phase_delay(self):
        ...
        for sources in [sources_rad, sources_deg]:
            # 1D array
            D1, DD1 = sources.phase_delay_matrix(
                sensor_locations_1, self.wavelength, True
            )
        ...
        # 1D array
        D1, DD1 = sources_sin.phase_delay_matrix(
            sensor_locations_1, self.wavelength, True
        )
        D1_expected = np.array(
            [
...
class TestFarField2D(unittest.TestCase):
    ...
    def test_basics(self):
        locations = np.array(
            [[0, 30], [0, 50], [135, 35], [150, 70], [-120, 60]], dtype=np.float_
        )
    ...
    def test_phase_delay(self):
        sources = FarField2DSourcePlacement(
            np.array([[27.0, 84.0], [-92.0, 68.0], [37.0, 17.0]]), unit="deg"
        )
        # 1D array
        ...
        # 2D array
        sensor_locations_2 = np.array([[0.0, 0.0], [0.0, 0.5], [0.5, 0.0], [0.5, 0.5]])
        D2 = sources.phase_delay_matrix(sensor_locations_2, self.wavelength)
        D2_expected = np.array(
            [
                [0.0, 0.0, 0.0],
                [1.490841e-1, -1.176144e0, 1.808045],
                [2.925939e-1, -4.107187e-2, 2.399357],
                [4.416780e-1, -1.217216e0, 4.207402],
            ]
        )
        ...
        D3_expected = np.array(
            [
                ...
                [2.925939e-1, -4.107187e-2, 2.399357e0],
                [1.490841e-1, -1.176144e0, 1.808045e0],
                [4.416780e-1, -1.217216e0, 4.207402e0],
                [3.124383e0, 2.912834e0, 9.185128e-1],
                [3.416977e0, 2.871762e0, 3.317869e0],
                [3.273467e0, 1.736690e0, 2.726558e0],
                [3.566061e0, 1.695618e0, 5.125914e0],
            ]
        )
    ...
    def test_spherical_coords(self):
        source_locations = np.array(
            [[-np.pi / 2, np.pi / 9], [np.pi / 3, np.pi / 6], [np.pi / 4, -np.pi / 3]]
        )
        ref_locations = np.array([[0.0, 0.0], [0.0, 1.0], [1.0, 1.0], [1.0, 0.0]])
...
class TestNearField2D(unittest.TestCase):
    ...
    def test_basics(self):
        source_locations = np.array([[100.0, 20.0], [-30.0, 40.0], [70.0, 24.0]])
    ...
    def test_phase_delay(self):
        source_locations = np.array([[30.0, 40.0], [50.0, -120.0], [80.0, 60.0]])
        sources = NearField2DSourcePlacement(source_locations)
    ...
    def test_spherical_coords(self):
        source_locations = np.array([[10.0, 10.0], [20.0, 20.0], [-10.0, 10.0]])
        sources = NearField2DSourcePlacement(source_locations)
