...
from doatools.performance import (
    crb_det_farfield_1d,
    crb_sto_farfield_1d,
    crb_stouc_farfield_1d,
)
...
class TestCRB(unittest.TestCase):
    ...
    def test_det_farfield_1d(self):
        ...
        sigma = 1.0
        n_snapshots = 100
        CRB_actual = crb_det_farfield_1d(
            ula, sources, self.wavelength, P, sigma, n_snapshots
        )
    ...
    def test_sto_farfield_1d(self):
        ...
        sigma = 1.0
        n_snapshots = 100
        CRB_actual = crb_sto_farfield_1d(
            ula, sources, self.wavelength, P, sigma, n_snapshots
        )
        CRB_expected = np.array(
            [
    ...
    def test_stouc_farfield_1d(self):
        ...
        sigma = 1.0
        n_snapshots = 100
        CRB_actual = crb_stouc_farfield_1d(
            ula, sources, self.wavelength, p, sigma, n_snapshots
        )
        CRB_expected = np.array(
            [
    ...
    def test_convergence_farfield_1d(self):
        ...
        sigma = 1.0  # 30 dB SNR
        n_snapshots = 10
        CRB_stouc = crb_stouc_farfield_1d(
            ula, sources, self.wavelength, p, sigma, n_snapshots
        )
        CRB_sto = crb_sto_farfield_1d(
            ula, sources, self.wavelength, p, sigma, n_snapshots
        )
        CRB_det = crb_det_farfield_1d(
            ula, sources, self.wavelength, p, sigma, n_snapshots
        )
