...
class TestWeightFunction(unittest.TestCase):
    ...
    def test_ula(self):
        ...
        npt.assert_array_equal(wf.differences(), np.array([-3, -2, -1, 0, 1, 2, 3]))
        npt.assert_array_equal(wf.weights(), np.array([1, 2, 3, 4, 3, 2, 1]))
    ...
    def test_coprime_array(self):
        ...
        npt.assert_array_equal(
            wf.weights(), np.array([1, 1, 2, 1, 2, 3, 3, 2, 6, 2, 3, 3, 2, 1, 2, 1, 1])
        )
        indices_expected = {
            -9: [30],
            -7: [31],
