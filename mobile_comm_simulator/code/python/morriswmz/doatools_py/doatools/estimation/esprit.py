    ...
    def estimate(
        ...
        if isinstance(row_weights, str):
            if row_weights == "none":
                ...
                raise ValueError(
                    "When specified using a string, row weights must be either 'none' or 'default'."
                )
        elif isinstance(row_weights, np.ndarray):
            if row_weights.ndim != 1 or row_weights.size != m_reduced:
                raise ValueError(
                    "Row weights must be a vector of length {0}.".format(m_reduced)
                )
        else:
            raise ValueError(
                "Row weights must be 'default', 'none', or a compatible numpy vector."
            )
        ...
        return True, FarField1DSourcePlacement.from_z(
            z, self._wavelength, d0 * displacement, unit
        )
