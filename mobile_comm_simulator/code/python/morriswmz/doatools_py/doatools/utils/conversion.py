...
ANGULAR_COV_MAT = {
    "rad": {"deg": lambda x: np.rad2deg(x), "sin": lambda x: np.sin(x)},
    "deg": {"rad": lambda x: np.deg2rad(x), "sin": lambda x: np.sin(np.deg2rad(x))},
    "sin": {"rad": lambda x: np.arcsin(x), "deg": lambda x: np.rad2deg(np.arcsin(x))},
}
