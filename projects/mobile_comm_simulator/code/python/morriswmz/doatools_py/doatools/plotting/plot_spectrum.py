...
def _build_spectrum_list(sp, grid):
    ...
    for t in sp_list:
        if t[0].shape != grid.shape:
            raise ValueError(
                "The shape of the spectrum, {0}, does not match the search grid, {1}.".format(
                    t[0].shape, grid.shape
                )
            )
    return sp_list
...
def plot_spectrum_1d(
    sp, grid, ax, estimates=None, ground_truth=None, use_log_scale=False, discrete=False
):
    ...
    for i, (spectrum, label) in enumerate(sp_list):
        ...
        if len(label) > 0:
            has_legend = True
        if discrete:
            container_sp = ax.stem(x, y, "-", markerfmt=" ", basefmt=" ", label=label)
    ...
    if estimates is not None:
        if estimates.units != grid.units:
            raise ValueError(
                "The unit of estimates does not match that of the search grid."
            )
        x_est = estimates.locations
        y_est = np.ones(x_est.shape)
        container_est = ax.stem(
            x_est, y_est, "--", markerfmt="x", basefmt=" ", label="Estimates"
        )
    ...
    if ground_truth is not None:
        if ground_truth.units != grid.units:
            raise ValueError(
                "The unit of ground truth does not match that of the search grid."
            )
        x_truth = ground_truth.locations
        y_truth = np.ones(x_truth.shape)
        container_truth = ax.stem(
            x_truth, y_truth, "--", markerfmt="o", basefmt=" ", label="Ground truth"
        )
...
def plot_spectrum(
    ...
    **kwargs
    ...
    if ax is None:
        ...
        new_plot = False
    if grid.ndim == 1:
        ret = plot_spectrum_1d(
            sp, grid, ax, estimates, ground_truth, use_log_scale, **kwargs
        )
    elif grid.ndim == 2:
        ret = plot_spectrum_2d(
            sp, grid, ax, estimates, ground_truth, use_log_scale, **kwargs
        )
    else:
