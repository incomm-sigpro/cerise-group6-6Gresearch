...
def ensure_covariance_size(R, array):
    ...
    if R.shape[0] != m or R.shape[0] != m:
        raise ValueError(
            ...
            "Expected shape is {0}. Got {1}".format((m, m), R.shape)
        )
...
def ensure_n_resolvable_sources(k, max_k):
    ...
    if k > max_k:
        raise ValueError(
            "Too many sources. Maximum number of resolvable sources is {0}".format(
                max_k
            )
        )
...
class SpectrumBasedEstimatorBase(ABC):
    ...
    def _estimate(
        ...
        if n_peaks < k:
            ...
            flattened_indices = np.ravel_multi_index(
                peak_indices, self._search_grid.shape
            )
            ...
            if refine_estimates:
                ...
                peak_indices = np.unravel_index(
                    flattened_indices, self._search_grid.shape
                )
                self._refine_estimates(
                    f_sp, estimates, peak_indices, refinement_density, refinement_iters
                )
            if return_spectrum:
    ...
    def _refine_estimates(self, f_sp, est0, peak_indices, density=10, n_iters=3):
        ...
        subgrids = self._search_grid.create_refined_grids_at(
            *peak_indices, density=density
        )
