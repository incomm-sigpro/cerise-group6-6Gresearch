...
class Test1DGrids(unittest.TestCase):
    ...
    def check_1d_grid_with_axes(
        self, grid, axis_expected, axis_names_expected, units_expected
    ):
    ...
    def test_far_field_1d_refinement(self):
        ...
        self.check_1d_grid_with_axes(
            refined_single, axis_expected, grid.axis_names, grid.units
        )
        # Multiple
...
class Test2DGrids(unittest.TestCase):
    ...
    def check_2d_grid_with_axes(
        self, grid, axes_expected, axis_names_expected, units_expected
    ):
        ...
        npt.assert_allclose(
            grid.source_placement.locations, cartesian(ax1_expected, ax2_expected)
        )
    ...
    def test_far_field_2d(self):
        ...
        for k, v in default_sets.items():
            ...
            self.check_2d_grid_with_axes(
                grid, (az_expected, el_expected), ("Azimuth", "Elevation"), (k, k)
            )

    def test_far_field_2d_refinement(self):
        # az: [0, 10, 20, 30, 40, 50]
        # el: [0, 20, 40]
        grid = FarField2DSearchGrid(
            start=(0, 0), stop=(60, 60), size=(6, 3), unit="deg"
        )
        # Single
        refined_single = grid.create_refined_grid_at((0, 1), density=4, span=2)
        axes_expected = (np.linspace(0, 20, 9), np.linspace(0, 40, 9))
        self.check_2d_grid_with_axes(
            refined_single, axes_expected, grid.axis_names, grid.units
        )
        # Multiple
        refined_multi = grid.create_refined_grids_at(
            [0, 3, 5], [0, 1, 1], density=4, span=1
        )
        axes_expected = [
            # (0, 0): (0, 0)
