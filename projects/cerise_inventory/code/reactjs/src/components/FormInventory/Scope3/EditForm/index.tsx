...
export function EditFormScope3() {
  ...
  const validationSchema = yup.object({
    ...
    passenger: yup.number().integer('O valor deve ser um número inteiro'),
    segment: yup.number().integer('O valor deve ser um número inteiro'),
  });
          ...
          .then(() => {
            inventoryItems.filter((item: any) => {
              if (item.id === inventoryItemId) {
                ...
                item.passenger = values.passenger;
                item.segment = values.segment;
              }
            });
  ...
  useEffect(() => {
    ...
    formik.values.description = JSON.parse(inventoryItem.description).text;
    formik.values.quantity = inventoryItem.quantity.replace(/\./g, ',');
    formik.values.passenger = inventoryItem.passenger;
    formik.values.segment = inventoryItem.segment;
    formik.values.origin = JSON.parse(inventoryItem.description).origin;
    formik.values.destination = JSON.parse(
