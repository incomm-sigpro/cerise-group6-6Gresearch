...
import { FormScope3 } from './Form';
import { EditFormScope3 } from './EditForm';
...
export function Scope3({ handleNext, handleBack }: Scope3Props) {
  ...
  const rows =
    rowsScope_03.length > 0
      ? rowsScope_03.map((item: any) => {
          return {
            id: item.id,
            description: JSON.parse(item.description).text,
            uf: item.uf,
            category: categories
              ? categories.find(
